...
function valoresTela3_2() {
  ...
  for (let i = 0; i < elemento.length; i++) {
    ...
    let corretaResposta = elemento[i].querySelector(
      ".corretaResposta input"
    ).value;
    let corretaURL = elemento[i].querySelector(".corretaURL input").value;
    let incorretaResposta1 = elemento[i].querySelector(
      ".incorretaResposta1 input"
    ).value;
    let incorretaURL = elemento[i].querySelector(".incorretaURL input").value;
    let incorretaResposta2 = elemento[i].querySelector(
      ".incorretaResposta2 input"
    ).value;
    let incorretaURL2 = elemento[i].querySelector(".incorretaURL2 input").value;
    let incorretaResposta3 = elemento[i].querySelector(
      ".incorretaResposta3 input"
    ).value;
    let incorretaURL3 = elemento[i].querySelector(".incorretaURL3 input").value;
    console.log(i);
    if (
      txtPergunta < 20 ||
      corretaResposta.length < 1 ||
      incorretaResposta1.length < 1
    ) {
